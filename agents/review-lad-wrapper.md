...
2. **Invoke `mcp__lad__code_review`.** Pass the dispatch payload's `paths` array as the tool's `paths` parameter; the tool resolves the references against the working tree at dispatch time. The wrapper invokes `mcp__lad__code_review` directly. If the upstream MCP server returns an error for any reason — missing/invalid API key, server crash, tool unavailable — the wrapper catches the failure and returns `status: blocked` with rationale `"LAD MCP tool invocation failed: <upstream error>"`. Third-party credential validation is the LAD MCP server's responsibility, not this wrapper's (Sprint Change Proposal 2026-05-18 § 4.1 + Story 12.2 — validation-responsibility-boundary correction).
...
4. **Observe the outcome and produce the envelope.** When the tool returns, observe (a) whether `code_review` ran successfully and the reviewers reached a verdict, (b) the surviving reviewer(s)' findings, (c) whether any upstream MCP-server error or precondition prevented the tool from running (MCP-process crash, MCP-tool timeout, upstream-reported error such as missing/invalid API key). Produce the envelope per the next section.
...
- `status` — one of `pass` | `fail` | `blocked`.
  ...
  - `blocked` — a precondition prevents review from running: the LAD MCP tool is unavailable; the upstream MCP server returned an error (the marker class is `LAD-skipped` per the Phase 1 marker taxonomy v1 closed-set; emission discipline lands in Story 10.5 + Story 12.2's validation-responsibility-boundary correction); the MCP process crashed or the tool timed out; or the dispatch payload is malformed.
...
- `rationale` — 1-3 sentences naming what the LAD reviewers reviewed and the verdict's basis. MUST be non-empty. On `status: blocked`, name the structural-precondition surface that fired. Examples by blocked sub-case:
  - MCP-tool-invocation-failure (any cause — upstream MCP-server error, including missing/invalid API key): "LAD MCP tool invocation failed: `<upstream error>`."
...
## API-key env-var hygiene (NFR-S1)

Per Sprint Change Proposal 2026-05-18 + Story 12.2's validation-responsibility-boundary correction, this wrapper does NOT read the OpenRouter API-key env var at runtime — neither for a presence check nor for value-fetching. The upstream `lad_mcp_server` MCP server is the single source-of-truth for credential validation: it reads `OPENROUTER_API_KEY` at server startup via the `claude mcp add ... -e OPENROUTER_API_KEY="$OPENROUTER_API_KEY" -- ...` install-handle env-var flow per ADR-008 line 681, and surfaces missing/invalid-credential errors at tool-invocation time through the natural `mcp__lad__code_review` failure path. The wrapper catches that failure per Procedure step 2 and returns `status: blocked` with rationale `"LAD MCP tool invocation failed: <upstream error>"`.

The dispatch payload may continue to carry an `api_key_env_var` field (per `SpecialistDispatchPayload.api_key_env_var`, retained as harmless infrastructure post-Story-12.2 — see Story 12.2 Completion Notes for the AC-6 IN-or-OUT judgment recording), but the wrapper does NOT read or act on it; the field's only remaining substrate consumer is the hygiene-witness test surface at `tools/loud-fail-harness/tests/test_api_key_hygiene.py`.

**Reading discipline (NFR-S1 structural enforcement, presence-check-independent):**
...
- The wrapper MUST NOT read the API key from `_bmad/automation/config.yaml` or from any other committed-to-git file. The `review_lad.api_key_env_var` config field carries the NAME of the env var, NEVER the key VALUE; the schema-validation contract per Story 10.4 AC-1 preserves this structurally.
- The wrapper MUST NOT introduce a fallback env var (e.g., reading `LAD_API_KEY` if `OPENROUTER_API_KEY` is unset). The legacy `LAD_API_KEY` placeholder was retired at Story 10.1 ADR-008 Consequence 3 ("not a backward-compatibility shim — pure replacement"); the upstream `lad_mcp_server` reads only `OPENROUTER_API_KEY`.

The structural witness for the never-in-files invariant is the substrate-library hygiene test at `tools/loud-fail-harness/tests/test_api_key_hygiene.py` (Story 10.5 AC-7) which byte-scans run-state YAML, specialist dispatch logs, evidence bundles, and PR-bundle markdown for the sentinel key-shaped string — regression on any persistence surface fails CI loudly. The hygiene invariant is structurally independent of the retired wrapper-side presence check.
