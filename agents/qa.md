...
6. Generate or reuse the `## QA Behavioral Plan` section in the story doc per FR23. Read `story_doc_text` from the canonical story-doc path (`_bmad-output/implementation-artifacts/{story-id}-*.md`). Call `persist_or_reuse_plan(story_doc_text, story_id, ac_list)` from `tools/loud-fail-harness/src/loud_fail_harness/qa_behavioral_plan.py` (the pure-library composition path; mirrors the dispatch-payload-renderer citation discipline used at line 16 above). Branch on the returned `PlanPersistAction` token:
   ...
   - `drift-suspected`: call `surface_plan_drift(parsed_plan, ac_list, story_id, registry)` from `tools/loud-fail-harness/src/loud_fail_harness/qa_plan_drift.py`. The library validates the `plan-drift-detected` marker against the runtime registry FIRST (atomic-on-failure per Pattern 5; registry rejection raises `UnknownMarkerClass`) AND returns a `PlanDriftEmission` carrying (a) a freshly-regenerated `QABehavioralPlan` whose `plan_status` is reset to `"generated"` regardless of prior state — even from `human-reviewed` it returns to `generated` per FR23, (b) a `PlanDriftEmissionRecord` carrying the canonical marker class identifier `"plan-drift-detected"` consumed AS-IS from `schemas/marker-taxonomy.yaml` line 114, and (c) a `PlanDriftDiagnosticContext` carrying the four required diagnostic fields verbatim (`story_id`, `prior_plan_status`, `prior_ac_hash`, `current_ac_hash`). Populate the QA envelope's optional top-level `plan_drift` field with the returned `diagnostic_context` (the schema bump at `schemas/envelope.schema.yaml`'s optional-extensions block is the structural carrier; the bundle assembler's QA-section render surfaces an `### Plan drift detected` H3 sub-section + the structured marker comment co-located when the field is non-null). Then call `validate_section_write("## QA Behavioral Plan")` from `tools/loud-fail-harness/src/loud_fail_harness/story_doc_validator.py`, assert `accepted is True`, render the fresh plan via `render_plan_section(emission.fresh_plan)`, and write it to the story doc under the `## QA Behavioral Plan` H2 header (same write path as the `write-new` branch).
   The library is pure: it does NOT perform file I/O, does NOT call `validate_section_write`, does NOT itself emit markers to the orchestrator-event log (the `PlanDriftEmissionRecord` is data the wrapper consumes; the structured bundle-comment marker is rendered by the bundle assembler when reading the envelope's `plan_drift` field). File reading + the section-allowlist check + the actual story-doc write + the envelope-field write are THIS wrapper's procedural-step responsibilities (sensor-not-advisor split per Pattern 5). Plan generation AND drift detection read ONLY `ac_list` from the dispatch payload + the parsed plan from the story doc — never TEA test files, dev tests, review findings, or commit diffs (FR16 invariant; structurally encoded by `tea_artifacts_consumed: []`).
