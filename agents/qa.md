...
6. Generate or reuse the `## QA Behavioral Plan` section in the story doc per FR23. Read `story_doc_text` from the canonical story-doc path (`_bmad-output/implementation-artifacts/{story-id}-*.md`). Call `persist_or_reuse_plan(story_doc_text, story_id, ac_list)` from `tools/loud-fail-harness/src/loud_fail_harness/qa_behavioral_plan.py` (the pure-library composition path; mirrors the dispatch-payload-renderer citation discipline used at line 16 above). Branch on the returned `PlanPersistAction` token:
   ...
   - `drift-suspected`: at THIS wrapper's Story-4.1 scope, treat as if no plan exists — call `validate_section_write` and write a freshly-regenerated plan, exactly as in the `write-new` branch. <!-- TODO(Story 4.2): consume `drift-suspected` here; emit the `plan-drift-detected` marker (already declared in `schemas/marker-taxonomy.yaml`) and reset `plan_status` to `generated` rather than regenerating fresh. The `qa_behavioral_plan.py` library's public API and the `PlanPersistAction` enum are forward-compatible with that thickening — no schema or library bump required. -->
   The library is pure: it does NOT perform file I/O, does NOT call `validate_section_write`, does NOT emit any marker. File reading + the section-allowlist check + the actual story-doc write are THIS wrapper's procedural-step responsibilities (sensor-not-advisor split per Pattern 5). Plan generation reads ONLY `ac_list` from the dispatch payload — never TEA test files, dev tests, review findings, or commit diffs (FR16 invariant; structurally encoded by `tea_artifacts_consumed: []`).
