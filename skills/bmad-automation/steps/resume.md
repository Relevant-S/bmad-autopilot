...
**Review-seam composition (Story 10.4 — `next_specialist=review-bmad` branch).** When `next_specialist == "review-bmad"`, do NOT thread directly to `steps/dispatch.md`'s single-specialist protocol; instead compose `loud_fail_harness.four_layer_review_dispatch.dispatch_four_layer_review(story_id=story_id, story_doc_resolution=..., run_state_path=..., dispatch_callback=dispatch_callback, event_log_appender=event_log_appender, marker_registry=registry, envelope_resolver=..., lad_enabled=<config["review_lad"]["enabled"] default False>, agent_definition_dir=...)`. Read `lad_enabled` from `_bmad/automation/config.yaml`'s `review_lad.enabled` key via the documented consumer-side default-handling pattern: `bool(config.get("review_lad", {}).get("enabled", False))` (Story 10.4 AC-1 `test_config_review_lad_omitted_defaults_disabled`). The substrate fans out to two `dispatch_callback` invocations when `lad_enabled=True` (one `specialist="review-bmad"`, one `specialist="lad"`) AND merges both envelopes via `merge_review_envelopes` into the single review-envelope-shape dict persisted to `final_run_state.last_envelope`. When `lad_enabled=False`, the substrate is bit-identical to a single `dispatch_callback(specialist="review-bmad", ...)` invocation (Story 10.4 AC-5 invariant). The `dev` and `qa` branches of `next_specialist` are UNCHANGED — they thread through to `steps/dispatch.md` directly.
