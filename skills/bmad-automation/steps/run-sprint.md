...
Invoke `init_sprint_run_state(sprint_id, run_id, epic_ids, unassigned_story_ids, multiplier=<resolved>)` to seed a `SprintRunState` at `current_state="sprint-in-progress"`, with `per_epic_status` seeded `epic-in-progress` per epic (pre-dispatch), `unassigned_story_ids` recorded in document order, the `per_sprint_retry_budget` **structure** populated (`effective_budget = multiplier × epic_count`, `consumed = 0`), and `active_markers=()`. (`run_sprint_loop` threads the multiplier into `init_sprint_run_state` for you.)

**Per-sprint budget ENFORCEMENT is Story 16.2, not here.** Story 16.1 populates the budget *structure* only, using `DEFAULT_PER_SPRINT_RETRY_MULTIPLIER = 2` as the seed. It does NOT enforce a per-sprint cumulative retry budget, does NOT compute an authoritative formula, does NOT emit `sprint-escalation-rate-exceeded`, and adds NO config fields. The sprint reaches `sprint-paused-on-budget` ONLY via a contained epic's own `epic-paused-on-budget` return (whose `epic-budget-exhausted` marker already lives at epic scope).
...
2. Recomputes the sprint `current_state` via the PURE `derive_sprint_state(per_epic_status, per_unassigned_status)` (folded in step 1).
3. Persists the advance via `advance_sprint_run_state` (durable markers survive the transient write-back filter).
4. Surfaces the AC-5 per-sprint framing line (see streaming below) via the `progress_sink`.
...
If folding a unit transitions the sprint to `sprint-paused-on-escalation` **OR** `sprint-paused-on-budget`, the loop STOPS: the downstream units do NOT auto-advance (AC-4). The transitions are:
...
Escalation keeps **precedence** over budget when both are present at the same boundary (mirroring `apply_epic_budget`'s precedence one scope down). The two sprint pause states are **distinct with different remediation paths**: `sprint-paused-on-escalation` = a *quality* issue (inspect the paused unit); `sprint-paused-on-budget` = a *cost* issue (raise the per-epic budget multiplier or split the epic, then re-dispatch). The orchestrator records the pause (terminal stream + the persisted sprint-run-state) and surfaces it, but does NOT itself decide whether to skip the paused unit and continue or hold the sprint — **that decision is the human's**. Flow policy that *halts* is in scope; flow policy that *resolves* a pause is not.
...
This story adds **ZERO** new marker-taxonomy entries and makes **ZERO** taxonomy version bump: `sprint-paused-on-budget` is reached only via a contained epic's `epic-paused-on-budget` (the `epic-budget-exhausted` marker already lives at epic scope); the per-sprint cumulative-budget marker and the `sprint-escalation-rate-exceeded` marker are Story 16.2's deliverables (PATCH bump there, not here).
...
- **Per-sprint budget enforcement → Story 16.2** (structure only here; reaches `sprint-paused-on-budget` via a contained epic's `epic-paused-on-budget`).
...
- Canonical Python composition: `bmad-autopilot/tools/loud-fail-harness/src/loud_fail_harness/sprint_lifecycle.py` (`run_sprint_loop`, `enumerate_sprint_units`, `init_sprint_run_state`, `derive_sprint_state`, `fold_epic_terminal`, `fold_unassigned_story_terminal`, `EpicLoopRunner`, `EpicLoopOutcome`, `SprintUnitEnumerationError`, `DEFAULT_PER_SPRINT_RETRY_MULTIPLIER`).
