...
- The story's `project_type` value read from `_bmad/automation/qa-runbook.yaml` (the `project_type: web | api` field at the practitioner's runbook stub Story 7.5 will scaffold; until Story 7.5 lands, the practitioner maintains the runbook stub manually).
...
(i) **Read project_type from the runbook stub.** Read `_bmad/automation/qa-runbook.yaml`'s `project_type` field as a string in `{"web", "api"}`. The substrate library does NOT read the runbook (FR16 invariant — env provisioning reads ONLY the project type from the runbook + the run-state's `story_id`; it does NOT read TEA test files, dev tests, review findings, or commit diffs). The LLM-runtime protocol owns this read so the substrate stays pure.
...
(iv) **Provision the env.** Call `provision_env(story_id, project_type, provisioner, port, run_state_path, registry, event_appender)`. The `provisioner` argument is a `Provisioner` Protocol implementation: Story 4.4 ships the web Playwright provisioner; Story 4.5 ships the API HTTP provisioner; THIS story's tests use `NoOpProvisioner`. On success the run-state's `provisioned_env` field is populated atomically (via the tempfile + `os.replace` discipline mirroring Story 2.2's `advance_run_state` pattern byte-for-byte) AND an `env-provisioned` event is appended to the events log. The function is atomic-on-failure — see step (v).
...
- **Marker class.** The `env-setup-failed` marker class (from `schemas/marker-taxonomy.yaml` line 102) is the canonical surface; the marker's `sub_cause` is one of the three values from `schemas/marker-taxonomy.yaml` lines 110-113 (`port-bind-failed`, `playwright-launch-failed`, `dev-server-not-ready`).
