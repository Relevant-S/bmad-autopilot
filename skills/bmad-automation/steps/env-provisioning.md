...
(ii) **Sweep stale orphans BEFORE fresh provisioning.** Call `cleanup_orphan_processes(run_state_path, orphan_probe, orphan_terminator, registry, event_appender, story_id=..., env_kind=project_type)`. This composes Story 4.3's project-type-agnostic orphan-detection seam — `orphan_probe.probe()` returns the list of `(port, pid)` pairs from prior crashed runs; `orphan_terminator.terminate(port, pid)` is invoked per pair; one `MarkerEmissionRecord` per orphan is constructed with `marker_class="orphan-process-cleanup"` + `context={"port": port, "pid": pid}` per the verbatim epic AC at epics.md line 1878; one `env-torn-down` summary event is emitted with `outcome="orphan-process-cleanup"`. On `UnknownMarkerClass` (registry-rejection), the function is atomic-on-failure — no terminator calls happen, no markers are constructed, no events are emitted; the rejection propagates to the orchestrator skill prompt. **Under `parallel_stories: true`** this sweep is the SAME Story 4.3 seam invoked per story against that story's per-worktree run-state (Story 14.4) — orphans are detected and the stale `provisioned_env` cleared per story, one `orphan-process-cleanup` marker per orphan port (NFR-S6), and two concurrent stories' sweeps are independent by per-worktree-run-state scoping (story A's probe/terminate never touches story B's port/pid). No new orphan-cleanup code path (Story 18.3 AC-5).

(iii) **Allocate a fresh ephemeral port.**

  - **Sequential mode (`parallel_stories: false`, the default) — unchanged.** Call `allocate_ephemeral_port()`. The function performs a single `socket.bind(("127.0.0.1", 0))` + `getsockname` + `close` sequence; it returns an integer in `[1, 65535]` from the OS's ephemeral range. The race-condition window between port-release and provisioner-bind is inherent to ephemeral allocation; the provisioner is expected to handle "port suddenly bound" by raising an exception that flows through `surface_env_setup_failure` with `sub_cause="port-bind-failed"` (handled by step (v) below).

  - **Parallel mode (`parallel_stories: true`) — consume the pre-allocated disjoint port (Story 18.3).** Do NOT call `allocate_ephemeral_port()` independently. The orchestrator's parallel claim provider (`loud_fail_harness.env_provisioning.ParallelEnvClaimProvider`, constructed by `run_epic_loop` on the dispatching thread) has already allocated a **disjoint** ephemeral port for this story via `loud_fail_harness.env_provisioning.DisjointPortAllocator` (re-rolling over `allocate_ephemeral_port()` so no two concurrently-live stories share a port — the prevention discipline Story 18.2's pollution detector is the sensor for) and **pre-seeded** it into this story's per-worktree run-state (the additive top-level `allocated_port` field; the per-story env namespace from `story_env_namespace(story_id)` is co-seeded as `env_namespace`). Read `allocated_port` from the per-worktree run-state and pass it as the `port` to step (iv) — so the port the detector registered in the live-claim registry is the exact port the dev server binds (no allocate-twice drift). The pre-seeded `env_namespace` prefix is **available** for the project-type provisioner to apply to the env vars it exports (provisioner adoption is forward-compatible — Stories 4.4/4.5/9.3 are unmodified).
...
(vii) **After QA return, tear down unconditionally.** When the QA Task-tool invocation returns (regardless of envelope `status` ∈ `{pass, fail, decision-needed, blocked}`), the orchestrator MUST call `teardown_env(provisioned_env, teardown_fn, run_state_path, evidence_root, registry, event_appender, story_id=...)` BEFORE making the next BMAD-lifecycle decision. Teardown is unconditional after QA completion per epics.md line 1869 — even on QA `fail` / `blocked` paths, the env is torn down BEFORE the orchestrator routes to retry / escalation. The teardown invariant: `teardown_fn.teardown(provisioned_env)` is invoked once; the run-state's `provisioned_env` field is cleared (the dev's-call YAML idiom is REMOVAL of the key per the extension-audit.md row); the `env-torn-down` event is emitted with `outcome="clean"`; the `evidence_root` directory and its contents are NEVER opened, listed, mutated, or referenced (Story 4.12 owns evidence persistence). **Under `parallel_stories: true`** teardown is per-story-isolated (Story 18.3 AC-4): `teardown_env` operates on this story's single `ProvisionedEnv` + its per-worktree run-state path, so tearing down story A clears ONLY A's `provisioned_env` and terminates ONLY A's PID, leaving sibling B's per-worktree run-state intact; the disjoint port A held is released back to the allocator at the dispatcher's per-story terminal-cleanup point (so a later wave may reuse it) without affecting B's still-held port.
...
- `loud_fail_harness.env_provisioning.DisjointPortAllocator` + `ParallelEnvClaimProvider` + `story_env_namespace` + `pre_seed_parallel_env` (Story 18.3 — the concurrent-provisioning discipline: disjoint pre-flight ports + per-story env namespace + the per-worktree-run-state carrier, constructed by `run_epic_loop` ONLY on the parallel branch; supplying the provider also activates Story 18.2's pollution detector)
