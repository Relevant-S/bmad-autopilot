...
### In-flight cost streaming MVP semantics — boundary-driven cadence + 75% threshold (Story 6.5 resolution)

Reference: NFR-O1 (PRD line 980) + NFR-O8 (PRD line 987) + NFR-P1 (PRD line 932) + ADR-006 Combo 3 + Story 6.4 cost-telemetry collection (the substrate Story 6.5 composes with). Builds on the rationale recorded for Story 6.4's H3 above (cross-reference; not duplicated here).

Story 6.5 lands the in-flight cost-streaming + 75%-threshold-detection substrate at `tools/loud-fail-harness/src/loud_fail_harness/cost_streaming.py` as a NEW SIBLING module composing with Story 6.4's `cost_telemetry.CostAggregation` (zero modifications to `cost_telemetry.py`). The MVP semantics extend the same boundary-driven cadence Story 6.4 established for cost-telemetry collection:

- **"In-flight" is explicitly framed as at specialist-return boundaries during a multi-specialist run** (per AC-1 verbatim — "any specialist returns from a dispatch event Then the running per-story cost ... streams to the main session output"), NOT during specialist execution. The cost-stream line `<HH:MM:SS> [cost] specialist=<kebab> retry=<n> delta=$<x.xx> total=$<y.yy> (<pct>% of $<ceiling.2f>)` is rendered ONCE per specialist-return boundary; the threshold check fires at the same cadence.
- **Gap between specialist-return boundaries is bounded by NFR-P2's per-specialist timeout (15-min default per Story 2.6 / `specialist_dispatch.py:1382-1383`).** A runaway specialist exceeding the ceiling between boundaries surfaces the warning at the next return boundary — the worst-case latency between threshold observations is one specialist-timeout window. This gap-tolerance is the MVP tradeoff against intra-specialist sampling.
- **100% ceiling crossing does NOT auto-halt the loop** per NFR-O8 verbatim ("Crossing the ceiling does not auto-halt the loop; the practitioner decides whether to abort"). The loud-fail doctrine reaches the cost-ceiling boundary as visibility-not-enforcement; auto-halt would make cost a hard constraint that violates the trust-the-practitioner UX posture. The warning text contains the literal substring `"no auto-halt per NFR-O8"` for grep-based regression checks and identifiability in non-color terminal contexts.
- **Phase 2 in-flight intra-specialist sampling is a forward-looking FR-P2 candidate revisited when Claude Code's cost-event cadence stabilizes** — parallel rationale to Story 6.4's H3 (above). Intra-specialist sampling depends on the same host-Bridge primitives (per ADR-006 Consequence 5) whose cadence is not yet pinned at MVP; the boundary-driven design is the MVP floor, not the ceiling.

The `cost-near-ceiling` marker class (taxonomy entry at `schemas/marker-taxonomy.yaml` lines 201-208 per Story 1.4's epic-close sweep) is consumed AS-IS; Story 6.5 makes ONE additive `sub_classifications` enum extension (`ceiling-crossed`) per Pattern 2's `: <cause>` suffix format — same shape Story 6.4 demonstrated for `cost-telemetry-unavailable`'s two sub-classifications. The marker emits with empty context (`pointer_context_fields: []` per Story 6.2's contract); the diagnostic-pointer text renders verbatim. Same-boundary 75%+100% jumps (the runaway-specialist scenario) emit BOTH markers in ascending-severity order per Story 1.4's marker-permanence rule.
...
## Three-Layer Parallel-Pass Thickening (Story 3.1)

This addendum records the Epic-3 in-place thickening of the Review-BMAD wrapper at `agents/review-bmad-wrapper.md`. Story 3.1 grows the wrapper's internal coverage from 1 layer (the Acceptance Auditor only, at Epic 2 scope) to 3 layers (Blind Hunter + Edge Case Hunter + Acceptance Auditor, at Epic 3 scope) WITHOUT changing the agent identity, the agent-definition filename, the envelope contract, or the Automator's dispatch substrate. The same five required envelope fields (`status` / `artifacts` / `findings` / `rationale` / `failed_layers`) Story 2.9 documented carry through unchanged; orchestrator-side consumers (Story 2.6's `validate_return_envelope`, Story 2.4's `evaluate_envelope`, Story 2.11's `assemble_bundle`) require zero changes for this thickening. Cross-references the existing "Acceptance Auditor — Epic 2 Single-Layer Rationale (Story 2.9)" section above; Story 3.5 produces the written audit artifact that validates the Story 2.9 rationale clauses (traceability + seam-contract churn minimization) against the actual three-layer composition this story lands.

**Reading A architectural commitment (verbatim from the Epic 2 retrospective at `_bmad-output/implementation-artifacts/epic-2-retro-2026-04-29.md` lines 71-72 + 110):** "Epic 3's 'parallel three-layer pass' means: orchestrator dispatches ONE Review-BMAD specialist via ONE Task-tool call. BMAD-core handles layer parallelism internally. Automator sees one dispatch, one envelope return. No concurrent write window, no `dispatch_parallel_specialists` needed, no Story 2.6 substrate surgery." The commitment encodes a cell-5 boundary discipline (compose, don't orchestrate-internally): the wrapper invokes `bmad-code-review` once at `review_mode = "full"` and lets the upstream skill's `step-02-review.md` run the three-layer parallel-pass invocation per its standard procedure (`_bmad/bmm/4-implementation/bmad-code-review/steps/step-02-review.md` lines 18-29); aggregation (dedup, classify, source-merge) happens inside `bmad-code-review`'s subagent context via `step-03-triage.md` lines 13-31; the wrapper consumes the post-triage finding list verbatim. ADR-002 cell 5 (Specialist Wrapper Binding) is the architectural anchor for this discipline; FR26-FR29 (architecture.md line 776 + line 1224) are the functional requirements that Story 3.1 realizes structurally.

**Post-aggregation testability pattern (from epic-2 retro line 113):** "Story 3.1's contract-coverage tests exercise post-aggregation envelope shape via fixturized aggregated envelopes — not dedup-logic correctness." The CI tests in `tools/loud-fail-harness/tests/test_review_bmad_wrapper.py` operate on three new fixtures at `examples/envelopes/{review-pass-three-layer.yaml, review-fail-three-layer-patch.yaml, review-blocked-partial-layer-failure.yaml}` that encode what an aggregated envelope looks like; the tests assert structural conformance against `schemas/envelope.schema.yaml` and the AC's per-fixture invariants. They do NOT test `bmad-code-review`'s dedup logic — that lives inside the upstream skill's subagent context (a Claude Code Task-tool-dispatched runtime), NOT inside the Automator's Python substrate. Same architectural pattern as Story 2.13's stub-dispatch-callback smoke test (the substrate test stubs out the actual Task-tool dispatch and feeds canned envelopes through the validation pipeline). This is review-enforced, NOT CI-enforced as a new gate — the wrapper's prose discipline gate per Story 3.1 AC-2 is the structural enforcement; the architecture.md addition is where the rationale lives for human readers.

**No new ADR.** The three-layer thickening is a structural realization of the existing FR26-FR29 commitment + ADR-002 cell 5 (Specialist Wrapper Binding) + ADR-004 (Task-tool dispatch); no new architectural decision is introduced. Existing ADRs are NOT renumbered; the existing "Acceptance Auditor — Epic 2 Single-Layer Rationale (Story 2.9)" section above is NOT modified. This addendum is additive.

## Finding-Taxonomy Passthrough (Story 3.2)

This addendum records the Epic-3 in-place thickening of the Review-BMAD wrapper's finding-taxonomy contract. Story 3.2 grows the wrapper at `agents/review-bmad-wrapper.md` by appending a named "Finding-taxonomy passthrough discipline (Story 3.2)" H2 section without changing the agent identity, the agent-definition filename, the envelope contract, or the Automator's dispatch substrate. The commitment is structural: BMAD core's finding taxonomy is the canonical source for `bucket` ∈ `{decision_needed, patch, defer, dismiss}` and `severity` ∈ `{HIGH, MED, LOW}` per FR27; the wrapper composes `bmad-code-review`'s `step-03-triage.md` step 3 output verbatim — without coercion, normalization, alias expansion, severity remapping, or supplement.

**Three artifacts agree at MVP scope.** The wrapper's prose is the human-readable contract; `schemas/envelope.schema.yaml` `$defs/finding`'s strict `bucket` enum (line 129) and `severity` enum (line 132) plus `additionalProperties: false` (line 110) are the structural enforcement; `validate_return_envelope` at the substrate seam is the runtime gate. The schema change-set is empty at this story (no enum bump; no shape extension) — the structural enforcement already exists; Story 3.2 makes the discipline an explicit named contract section + tested invariant. Forward-compatibility is loud, not silent: an envelope carrying a finding with an out-of-enum bucket or severity value, or an extra classification field, is REJECTED at `validate_return_envelope` per Pattern 5 (loud-fail doctrine). The operational response — bumping `envelope.schema.yaml`'s enum AFTER recording the BMAD-extension event in `docs/extension-audit.md`'s "Finding-taxonomy bumps — the no-introductions principle" subsection — is the architectural backstop that makes silent drift impossible.

**Downstream consumer — Story 5.2 retry-router (`epics.md` lines 2264-2289).** Story 5.2 consumes the `bucket` field as a structural routing key directly, with no extra parsing or re-derivation: `patch` (HIGH/MED) → Dev fix-only retry per FR9 with derived action items; `decision_needed` → escalate; `defer` → record to `deferred-work.md` and advance state; `dismiss` → already dropped by `step-03-triage.md` step 4 (the wrapper does NOT coerce or drop a `dismiss` finding if one ever survives upstream triage — that is upstream's job). The Story 3.2 passthrough invariant is what makes the Story 5.2 four-row routing rule lift directly off `bucket` without re-derivation; the wrapper exposes the bucket signal, Story 5.2 implements the routing.

**Cross-references.** This addendum is a clarification of the existing FR27 + FR62 commitments, NOT a new architectural decision. No new ADR is introduced; existing ADRs are NOT renumbered; the existing "Three-Layer Parallel-Pass Thickening (Story 3.1)" section above is NOT modified. The operational policy companion lives at `docs/extension-audit.md` ("Finding-taxonomy bumps — the no-introductions principle"); the structural enforcement lives at `schemas/envelope.schema.yaml` (`$defs/finding`); the named contract section lives at `agents/review-bmad-wrapper.md` ("Finding-taxonomy passthrough discipline (Story 3.2)"). Epic 3 introduces ZERO new taxonomy values — the no-introductions invariant is established by THIS story.

## Failed-Layers Three-Channel Surface (Story 3.3)

This addendum records the Epic-3 in-place thickening of the Review-BMAD wrapper's per-layer-failure surface. Story 3.3 graduates the wrapper-side handling of `failed_layers` from a single-channel envelope-field declaration (Story 3.1) to a three-channel atomic emission whose three projections (envelope field for the orchestrator per FR56; `review-layer-failed` marker for tooling per FR28; synthetic `decision_needed: HIGH` finding carrying `meta: review-completeness` for the human reviewer) are emitted by a single source-of-truth function `surface_failed_layers` at `tools/loud-fail-harness/src/loud_fail_harness/review_layer_failure.py`. The three projections MUST agree by construction; mismatch is a contract violation Epic 6 / Story 6.1's cross-channel reconciliation gate surfaces.

**Three-channel atomic emission contract.** A failed review layer is a fact that has THREE projections, each addressing a different consumer audience: (1) the orchestrator reads `failed_layers` for flow-policy routing per FR52; (2) downstream tooling (Story 1.5's `enumeration_check` + Story 1.8's `fr33-fixture-gate` + Story 3.4's PR-bundle review-section rendering at `tools/loud-fail-harness/src/loud_fail_harness/bundle_assembly.py`) consumes the rendered `<!-- bmad-automation:marker review-layer-failed: <layer> -->` HTML-comment marker; (3) the human reviewer reads the synthetic `decision_needed: HIGH` finding distinguished by the `meta: review-completeness` discriminator (Story 3.4's downstream filtering target per `epics.md` line 1729). Each per-layer failure produces ONE projection per channel — `len(failed_layers)` markers and `len(failed_layers)` synthetic findings; the zero-failure path is silent at channels 2 + 3 (no marker; no synthetic finding).

**Schema bump as worked instance of Story 3.2's no-introductions principle.** Story 3.3 lands the FIRST worked instance of Story 3.2's "Finding-taxonomy bumps — the no-introductions principle" subsection at `docs/extension-audit.md` lines 124-134: the `meta: review-completeness` field is a wrapper-layer-only addition recorded in the per-convention table (classification `automator-internal` by analogy to the `failed_layers` graceful-degradation field row precedent at line 33) BEFORE the schema enum is bumped at `schemas/envelope.schema.yaml`'s `$defs/finding.properties.meta` (single-value enum `[review-completeness]`; future meta-finding kinds extend the enum via a follow-on schema bump recorded in the same audit-doc subsection). The schema's `$defs/finding.additionalProperties: false` invariant is PRESERVED — `meta` is added as a NAMED optional property, not a relaxation of the closed-by-default shape; Story 3.2's forward-compat loud-fail tests (rejection of `category` / `subcategory` / `tags` extras) continue to pass.

**Atomicity is a code-structure invariant.** The three-channel agreement is enforced as a code-structure invariant (a single source-of-truth function is the only emission path; a CI lint at `tools/loud-fail-harness/src/loud_fail_harness/review_layer_failure_emission_gate.py` scans the harness source tree + the wrapper agent definition for forbidden direct mutations of any of the three channels outside this function), NOT a per-bundle reconciliation gate. This is the SAME pattern Story 2.2 applied to atomic-write: NFR-R8 ("write-ordering invariant") is enforced by the API shape of `RunStateAtomicWriter.write`, not by developer discipline. The CI lint mirrors Story 1.10a's `pluggability_gate.py` regex-rule pattern at lines 113-145; the gate is added to the harness CI sweep at `.github/workflows/ci.yml` as the now-13th harness CI command after `pluggability-gate`.

**Cross-references.** This addendum is a clarification of FR28 + FR56 + the no-introductions principle, NOT a new architectural decision. No new ADR is introduced; existing ADRs are NOT renumbered; the existing "Three-Layer Parallel-Pass Thickening (Story 3.1)" + "Finding-Taxonomy Passthrough (Story 3.2)" sections above are NOT modified. The operational policy companion lives at `docs/extension-audit.md` ("Finding-taxonomy bumps — the no-introductions principle" subsection + the new `meta: review-completeness` per-convention table row); the structural enforcement lives at `schemas/envelope.schema.yaml` (`$defs/finding.properties.meta`); the named contract section lives at `agents/review-bmad-wrapper.md` ("Failed-layers three-channel surface (Story 3.3)"); the code-structure invariant lives at `tools/loud-fail-harness/src/loud_fail_harness/review_layer_failure.py` (the canonical emission home) + `review_layer_failure_emission_gate.py` (the CI lint enforcing the no-other-emission-path rule). The cross-channel reconciliation downstream consumer is Epic 6 / Story 6.1; the PR-bundle review-section rendering downstream consumer is Story 3.4; THIS story is the producer of the three-channel agreement.

## PR-Bundle Review-Section Rendering — Bucket × Severity Grouping (Story 3.4)

This addendum records the Epic-3 in-place thickening of the PR-bundle review-section renderer at `tools/loud-fail-harness/src/loud_fail_harness/bundle_assembly.py`'s `_render_review_findings_section`. Story 3.4 graduates the rendering shape from a single-layer flat findings list (Story 2.11 + 3.3 era) into a bucket × severity grouped surface that preserves layer attribution AND visually distinguishes Story 3.3's synthetic `meta: review-completeness` meta-findings (the Story 3.3 channel-3 surface) from layer-produced content findings (the Blind Hunter / Edge Case Hunter / Acceptance Auditor outputs the Story 3.1 three-layer pass aggregates). Layer-produced findings render under `### bucket: <name>` H3 sub-sections in fixed canonical order `(decision_needed, patch, defer, dismiss)` per FR27, with `**HIGH:**` / `**MED:**` / `**LOW:**` severity sub-headers nested within each non-empty bucket; each bullet carries the finding's `source` layer name as a `[<layer>]` prefix tag preserving the Story 3.1 layer-attribution discipline through to human reading. Synthetic meta-findings render in a dedicated `### Review-completeness meta-findings (synthetic; per Story 3.3)` sub-section appearing AFTER the bucket × severity sections.

**In-place flag flip per the Story 2.11 in-place-flip pattern.** `tools/loud-fail-harness/src/loud_fail_harness/thickening_flags.py`'s `is_full_review_present()` flips from `return False` to `return True` per the verbatim epic AC at epics.md Story 2.11 lines 1521-1522 ("each downstream epic flips its corresponding flag when its thickening lands"). The flip cascades automatically through `_render_walking_skeleton_header` (the renderer iterates `_THICKENING_SENTENCES` and skips entries whose flag returns `True`); the dropped sentence "Single-layer review (Epic 3 thickens to 3-layer adversarial pass)." is the structural witness of the flip. No edit to the header-rendering function or to `_THICKENING_SENTENCES` is required — the flag flip is the sole producer of the cascading behavior change.

**Retained-marker rationale — the structural-not-era-based emission rule.** The `walking-skeleton-bundle` marker continues to emit at Epic 3. The emission rule, established at Story 2.11 AC-4, is structural: emit iff `is_loud_fail_block_present()` returns `False`. At Epic 3 substrate state `is_loud_fail_block_present()` continues to return `False` (Epic 6 / Story 6.1 owns the loud-fail block landing that flips the flag), so the marker continues to emit. The rule is documented at `marker-taxonomy.yaml` lines 210-216's diagnostic_pointer prose ("absent loud-fail block triggers the marker, NOT 'Epic 2 era triggers the marker' (rule clarified by Epic 6).") and re-stated at `bundle_assembly.py`'s module docstring lines 41-52. Story 3.4 is the FIRST downstream test of the rule's structural posture: the marker stays at Epic 3 because the predicating flag stays `False`; Epic 6 / Story 6.1 inverts emission via the flag flip without any edit to the bundle-assembly rendering code. The Story 2.11 AC-4 regression baseline `test_walking_skeleton_marker_suppressed_when_loud_fail_block_present` (which substitutes a stub flag namespace) is the structural witness that the inversion fires correctly when Epic 6 lands the production flag flip.

**Producer-consumer pair with Story 3.3.** Story 3.3 produced the three-channel surface (envelope `failed_layers` field for the orchestrator + `<!-- bmad-automation:marker review-layer-failed: <layer> -->` per-layer marker for tooling + synthetic `decision_needed: HIGH meta: review-completeness` finding for the human reviewer). Story 3.4 is the bundle-rendering consumer of channel 3: the `meta` discriminator is the partitioning key that segregates synthetic meta-findings into the dedicated sub-section. The wrapper-side emission contract is unchanged at THIS story — Story 3.3 owns the wrapper's three-channel surface; Story 3.4 owns the bundle-side rendering. The Story 3.3 AC-4 invariant (per-failed-layer `validate_marker_emission` defense-in-depth call; one marker comment per failed layer) is preserved verbatim in shape; the `review-layer-failure-emission-gate` CI lint continues to allowlist the canonical site at `_render_review_findings_section`.

**Cross-references.** This addendum is a clarification of the existing FR27 + FR28 + FR56 + Story 2.11 AC-4 commitments, NOT a new architectural decision. No new ADR is introduced; existing ADRs are NOT renumbered; the existing "Three-Layer Parallel-Pass Thickening (Story 3.1)" + "Finding-Taxonomy Passthrough (Story 3.2)" + "Failed-Layers Three-Channel Surface (Story 3.3)" sections above are NOT modified. FR27 (`_bmad-output/planning-artifacts/prd.md`) is the canonical-source authority for the bucket × severity passthrough; `marker-taxonomy.yaml` lines 210-216 + `bundle_assembly.py` module docstring lines 41-52 are the canonical sources for the structural-not-era-based marker emission rule; the canonical example bundle fixture at `examples/pr-bundles/pr-bundle-walking-skeleton.md` is regenerated in place per the Story 2.11 AC-7 precedent and the contract-coverage test `test_canonical_walking_skeleton_bundle_fixture_matches_assembler_output` is updated in place per AC-5. Epic 3 introduces ZERO new schema bumps at THIS story — the rendering thickening uses the existing Story 3.3 `meta: review-completeness` enum verbatim. The downstream consumer is Story 3.5 (Acceptance Auditor rationale validation reads the actual three-layer rendered surface for its written audit artifact); Epic 6 / Story 6.1 is the consumer of the structural-not-era-based posture that earns its keep when `is_loud_fail_block_present()` flips and the marker emission inverts without any edit to the rendering code.

## QA Behavioral Plan AC-Hash Function — Stability Guarantees (Story 4.1)

This addendum records the stability guarantees of `compute_ac_hash` at `tools/loud-fail-harness/src/loud_fail_harness/qa_behavioral_plan.py`, the deterministic hash function consumed by Story 4.2's drift-detection surface and surfaced in every persisted `## QA Behavioral Plan` section's `<!-- ac_hash: ... -->` metadata header. Per the verbatim epic AC at `_bmad-output/planning-artifacts/epics.md` line 1856 ("the AC-hash function is documented in `docs/architecture.md` with its stability guarantees"), THIS addendum is the canonical declaration of the hash function's behavior; the module docstring is the in-code restatement; the test suite at `tools/loud-fail-harness/tests/test_qa_behavioral_plan.py` is the structural witness.

**Canonical normalization rules.** The hash is computed over a canonical normalized string representation of the `ac_list`. (1) For each AC entry, only the `ac_text` is consumed — the `ac_id` is the index, NOT the content. (2) Each `ac_text` is normalized: internal whitespace runs collapse to a single space (`re.sub(r"\s+", " ", text)`); leading/trailing whitespace is stripped (`text.strip()`). (3) The normalized AC texts are sorted lexicographically. (4) The sorted normalized texts are joined with a single newline (`\n`) separator. (5) The joined string is encoded UTF-8. (6) `hashlib.sha256(...).hexdigest()` returns the full 64-character hex digest.

**Determinism guarantee.** Two calls to `compute_ac_hash` with the same `ac_list` content yield byte-identical digests across runs and across processes. The function is pure: no global state mutation, no I/O, no clock dependency, no PYTHONHASHSEED dependency (the normalization sorts before joining; Python `str` sort is deterministic; `hashlib.sha256` is deterministic by definition). The structural witness is `test_compute_ac_hash_deterministic`.

**Whitespace stability guarantee** (epics.md line 1832). Two calls with `ac_text` differing only in whitespace runs (spaces, tabs, newlines, mixed) yield identical digests. The normalization step's `re.sub(r"\s+", " ", text).strip()` collapses any whitespace shape (Unicode `\s` class) to a single space and strips edges; the canonical string is invariant under whitespace-only edits to the AC text. The structural witness is `test_compute_ac_hash_whitespace_stable`. Practical consequence: editorial reflows (line wrapping, indentation changes, trailing newline differences) do NOT trigger drift in Story 4.2 — only substantive AC-text changes do.

**Content discrimination guarantee.** Two calls with `ac_text` differing by any non-whitespace character yield different digests (single-character substitution produces a SHA-256 digest with avalanche-property dispersal). The structural witness is `test_compute_ac_hash_content_discriminates`. Practical consequence: any user-visible AC text edit propagates as drift to Story 4.2's surface; the function does NOT silently swallow content changes.

**Order-stability decision** (the dev's-call covered by epics.md line 1855: "AC reordering with same content does not trigger drift if hash is order-stable per the documented hash function"). Story 4.1 chose order-stable. The normalization step sorts the per-entry normalized texts lexicographically before joining — AC reordering with the same content yields the same canonical string and therefore the same digest. Rationale: (a) the user-visible identity of an AC is its text, not its position in `## Acceptance Criteria`; (b) editorial reorderings are common (smoke-first reordering, severity reordering) and should not invalidate a `human-reviewed` plan; (c) Story 4.6's smoke-first ordering operates on plan entries, not on the `ac_list` input to the hash, so the order-stable choice does not collide with FR22b's smoke-first commitment. The structural witness is `test_compute_ac_hash_order_treatment`. If a future story discovers that order matters for drift signaling, the decision is reversible via a single-line edit to `_normalize_ac_text` callers (drop the `sorted(...)`); the architecture.md addendum and the test suite enforce whichever decision lands.

**Choice of `hashlib.sha256` over alternatives.** SHA-256 was chosen for collision-resistance margin (64-char hex digest; 2^128 birthday-bound), Python stdlib availability (no new runtime dependency per Pattern 6 + the pinned-tooling discipline at `pyproject.toml`), and broad ecosystem familiarity. MD5 / SHA-1 were rejected for known collision attacks. SHA-512 was rejected as gratuitous (the hash is consumed by exact-equality comparison; collision-resistance margin beyond SHA-256 is not load-bearing for drift detection). BLAKE2 / BLAKE3 were rejected for ecosystem-familiarity reasons (SHA-256 is the default cognitive choice for "stable content hash" across the team's prior context).

**Consumer linkage forward-pointer.** Story 4.2's drift-detection surface consumes this hash as its single source of equality truth. The contract is stable from Story 4.1 onward: (a) the `ac_hash` field on a persisted `QABehavioralPlan` is invariant under whitespace and reordering edits to `ac_list`, sensitive to substantive AC-text edits; (b) `compute_ac_hash(ac_list)` evaluated against the persisted `ac_hash` is the drift signal — equality means no drift, inequality means drift. Story 4.2 adds the `plan-drift-detected` marker emission + `plan_status` reset on top of the `drift-suspected` `PlanPersistAction` token Story 4.1 already returns; Story 4.2 does NOT redefine the action enum, does NOT change the hash function, does NOT redefine these stability guarantees. The forward-compatibility commitment is structural.

## QA Wrapper Thickening Completion + `is_full_qa_present` Flag Flip (Story 4.13)

This addendum records the Epic-4-close in-place thickening of the QA specialist's wrapper-side composition at `bmad-autopilot/agents/qa.md` and the cascading `is_full_qa_present()` flag flip at `tools/loud-fail-harness/src/loud_fail_harness/thickening_flags.py`. Story 4.13 is the FOURTEENTH and final wrapper-side primitive in Epic 4's QA-thickening sequence — closing the FR16-FR25 substrate-level claim block for the QA-specialist surface — AND the THIRD worked instance of the in-place-flip pattern Story 2.11 AC-2 ratified (after Story 3.4's `is_full_review_present` flip + Stories 4.1-4.12's cumulative landing of the eleven QA-domain pure-library modules — the "preparing the substrate" half; THIS story is the "fan-in flip" half).

**FR16-FR25 surface deltas — the wrapper-side composition.** The thickened `agents/qa.md` composes against the eleven QA-domain pure-library modules + `specialist_dispatch.py`'s registry primitives AS-IS without modification. The procedure section grows from the Story-2.10-era ~8-step AC-1-only / Tier-1-only minimum to the post-4.13 ~13-step full-FR16-FR25 surface naming each module by its fully-qualified `tools/loud-fail-harness/src/loud_fail_harness/<module>.py` path: `qa_behavioral_plan` (4.1) for plan creation + persistence, `qa_plan_drift` (4.2) for AC-hash drift detection + `plan_status` reset, `env_provisioning` (4.3) for the orchestrator-owned env-lifecycle record QA reads, `playwright_driver` (4.4) for the `web` project-type driver, `http_driver` (4.5) for the `api` project-type driver, `qa_ac_iteration` (4.6) for plan-driven AC iteration with smoke-first ordering, the schema-side `$defs/ac_result` `if/then` invariant (4.7) for AC-assertion-evidence triple structural enforcement, `qa_evidence_tier` (4.8) for the three-tier hierarchy + `surface_tier_3_not_configured`, `qa_exploratory_heuristics` (4.9) for the three MVP heuristics + `verification_mode` discriminator, the wrapper-side declaration shapes (4.10) for the two escalation contracts (verification-fail per FR24a; env-setup-fail per FR24b), `qa_plan_persistence_compromise` (4.11) for the visibility blockquote, and `qa_evidence_persistence` (4.12) for the canonical-path + size-budget + sanitization mechanism. The `ac_results` array's cardinality grows from the Epic-2 "exactly one entry" to the post-4.13 "one entry per iterated AC in story-doc order" (smoke-first abort produces a single `AC-1` entry whose `status: fail`); `semantic_verification` thickens from the literal string `"not_applicable"` to the object form `{tier: 1 | 2 | 3, status: configured | not_configured | not_applicable}` per the schema's `oneOf: [object, string]` allowance. The optional top-level QA-extension fields (`plan_drift`, `plan_persistence_compromise`, `marker_emissions`, `heuristic_skipped_emissions`) are populated when the corresponding QA-domain primitives surface them.

**In-place-flip pattern — third worked instance.** The verbatim Story 2.11 AC at `_bmad-output/planning-artifacts/epics.md` lines 1521-1522 mandates: "the assembler renders the header text by enumerating which thickening features are still missing" + "each downstream epic flips its corresponding flag when its thickening lands". Story 3.4 was the first worked instance (`is_full_review_present` flipped at the close of Epic 3); Stories 4.1-4.12 cumulatively prepared the substrate (the eleven QA-domain pure-library modules' stabilized public APIs); Story 4.13 is the fan-in flip closing Epic 4. Three surgical edits at the substrate-layer (the agent-definition file's wrapper-side composition; the flag-flip; the cascading rendering); two regenerated fixtures; one test rename + assertion update; two new tests; one appended audit-doc row; one appended architecture-doc H2 section (THIS section). NO new substrate module added; NO new schema bump; NO new agent definition.

**Cascading Walking-Skeleton-Mode header — structural-not-era-based posture.** NO edit to `tools/loud-fail-harness/src/loud_fail_harness/bundle_assembly.py::_render_walking_skeleton_header` is required for the post-4.13 substrate state. NO edit to the `_THICKENING_SENTENCES` tuple is required either. The flip cascades automatically through the renderer's iteration logic (`for flag_name, sentence in _THICKENING_SENTENCES: if not flag_fn(): missing.append(sentence)`); with `is_full_qa_present()` now returning `True` (joining `is_full_review_present()` since Story 3.4), the "Tier-1 evidence only (Epic 4 thickens to Tier-2 + Tier-3-where-configured)." sentence is OMITTED from the rendered header body. The dropped-sentence invariant is the structural witness of the flag flip — the third worked instance after Story 3.4's "Single-layer review" omission and Story 6.1's forthcoming "No loud-fail block" omission. The post-4.13 substrate state's rendered header enumerates EXACTLY TWO missing-thickening sentences ("No retry …" + "No loud-fail block …").

**Retained `walking-skeleton-bundle` marker emission rationale.** Epic 4 is NOT the era that triggers `walking-skeleton-bundle` marker suppression — Epic 6 / Story 6.1 owns. Per the verbatim Story 4.13 AC at `epics.md` lines 2203-2204: "`walking-skeleton-bundle` continues to emit because `is_loud_fail_block_present()` is still false (Epic 6 inverts that condition)" + "Epic 4's thickening does not suppress the marker — Epic 4 does not own the structural condition that drives marker emission". The emission rule is structural per Story 2.11 AC-4 (emit iff `is_loud_fail_block_present()` returns `False`) + per `marker-taxonomy.yaml` lines 210-216's diagnostic_pointer prose ("absent loud-fail block triggers the marker, NOT 'Epic 2 era triggers the marker' (rule clarified by Epic 6).") + per `bundle_assembly.py`'s module docstring lines 41-52. The Story 2.11 AC-4 regression baseline `test_walking_skeleton_marker_suppressed_when_loud_fail_block_present` (which substitutes a stub flag namespace where `is_loud_fail_block_present()` returns `True`) continues to be the structural witness that the inversion fires correctly when Epic 6 / Story 6.1 lands the production flag flip.

**FR62 pluggability gate continues to exit 0.** The thickened `agents/qa.md` honors Rule 1 (path-form: `agents/<other>.md` constructs are absent — the only path-form occurrences reference `agents/qa.md` itself, which the gate excludes as self-references) AND Rule 2 (slug-form: bare `dev-wrapper` / `review-bmad-wrapper` slugs are absent — sibling specialists are referenced by HUMAN-READABLE names "the Dev specialist", "the Review-BMAD specialist", "the Phase-1.5 LAD layer"). Module-path references like `tools/loud-fail-harness/src/loud_fail_harness/<module>.py` are NOT specialist references and so do NOT trigger Rule 1 (which fires only on `agents/<slug>...` constructs per `pluggability_gate.py` line 115-128). The `pluggability-gate` reports "3 passing specialist(s), 0 cross-reference violation(s) across 0 specialist file(s)" both pre- and post-4.13.

**Post-4.13 substrate state — TWO flags True; TWO flags False.** The `tools/loud-fail-harness/src/loud_fail_harness/thickening_flags.py` four-flag namespace at the post-4.13 substrate state: `is_full_review_present()` returns `True` (Story 3.4 flipped); `is_full_qa_present()` returns `True` (Story 4.13 flipped — THIS story); `is_retry_present()` returns `False` (Epic 5 / Story 5.1+ owns the flip when the whole-story retry budget + bucket-driven action item derivation lands); `is_loud_fail_block_present()` returns `False` (Epic 6 / Story 6.1 owns the flip when the loud-fail block + per-specialist × per-retry cost breakdown + actionable how-to-enable pointer enrichment lands). The module-level docstring is RELAXED IN PLACE per the Story 3.4 review-finding precedent — the substrate-state paragraph reflects the post-4.13 cumulative flag-flip state; the in-place-flip-pattern paragraph is preserved verbatim. The contract-coverage matrix at `tests/test_thickening_flags.py`'s module docstring is updated to name `test_thickening_flags_at_epic_4_substrate_state` (renamed from `test_thickening_flags_at_epic_3_substrate_state` per the Story 3.4 AC-7 precedent); the assertion-set updates from "1 True / 3 False" (post-3.4) to "2 True / 2 False" (post-4.13).

**Epic 4 epic-close marker sweep — no new marker classes.** The seven QA-domain marker classes (`evidence-truncated`, `playwright-mcp-unavailable`, `dev-server-not-ready`, `env-setup-failed`, `tier-3-not-configured`, `plan-drift-detected`, `heuristic-skipped`) were enumerated by Story 1.4's epic-close marker sweep and emitted ahead of THIS wrapper-side composition. Per the AS-IS-marker-class-reuse pattern Stories 4.6 (`smoke-first-abort` AS-IS) / 4.8 (`tier-3-not-configured` AS-IS) / 4.12 (`evidence-truncated` AS-IS) established, THIS story's wrapper composes against the existing classes by reference; NO `marker-taxonomy.yaml` bump is required at THIS story.

**Cross-references.** This addendum is a clarification of the existing FR16-FR25 + FR62 + Story 2.11 AC-2 + AC-4 commitments, NOT a new architectural decision. No new ADR is introduced; existing ADRs are NOT renumbered; the existing "QA — Epic 2 AC-1-Only Tier-1-Evidence-Only Rationale (Story 2.10)" + "Bundle Assembly — cell-1 contract / cell-4 host-Bridge boundary (Story 2.11)" + "Three-Layer Parallel-Pass Thickening (Story 3.1)" + "PR-Bundle Review-Section Rendering — Bucket × Severity Grouping (Story 3.4)" + "QA Behavioral Plan AC-Hash Function — Stability Guarantees (Story 4.1)" sections above are NOT modified. The wrapper-side composition contract lives at `agents/qa.md`; the substrate-side flag namespace lives at `tools/loud-fail-harness/src/loud_fail_harness/thickening_flags.py`; the structural-not-era-based marker emission rule lives at `marker-taxonomy.yaml` lines 210-216 + `bundle_assembly.py` module docstring lines 41-52; the audit-doc per-convention table row at `docs/extension-audit.md` is the operational policy companion; the contract-coverage tests live at `tests/test_thickening_flags.py` + `tests/test_bundle_assembly.py`; the regenerated PR-bundle fixtures at `examples/pr-bundles/pr-bundle-walking-skeleton.md` + `examples/pr-bundles/pr-bundle-plan-drift.md` are the regression baselines against the Story 4.13 rendering delta. Epic 4 reaches `done` status once THIS story commits and the (optional) epic-retrospective ships; Epic 5 (retry surface; `is_retry_present` flip) + Epic 6 (loud-fail block; `is_loud_fail_block_present` flip + `walking-skeleton-bundle` marker emission inversion) are the next downstream consumers of the post-4.13 substrate state.

### Runtime evidence-trace linkability — bundle-render-time validation + dangling-evidence-ref sub-classifications (Story 6.6 resolution)

This addendum records the Story 6.6 bundle-render-time evidence-trace linkability validation surface at `tools/loud-fail-harness/src/loud_fail_harness/evidence_linkability.py`, the corresponding additive seams in `bundle_assembly.py` / `bundle_assembly_escalation.py`, and the additive `sub_classifications: [qa-evidence, retry-history]` extension to the `dangling-evidence-ref` entry at `schemas/marker-taxonomy.yaml:227`. Story 6.6 is the SIXTH Epic-6 landing — the runtime complement to Story 1.7's fixture-time + Story 5.5's CLI-hook-time dangling-evidence-ref surfaces.

1. **NFR-O7 invariant verbatim** (per `prd.md:986`): "Every `evidence_refs` entry resolves to a real artifact or emits a dangling-ref loud-fail marker." Story 6.6 enforces this at bundle-render time by walking every `ac_results[].evidence_refs[].path` in QA's envelope AND every thickened `run_state.retry_history[].path` against the on-disk artifact via `pathlib.Path.exists()`.

2. **Same `dangling-evidence-ref` marker class handles BOTH sources** per the remediation-shape principle ratified by Story 1.11 and extended by Story 5.5. Remediation is "find or restore the missing artifact" regardless of whether the missing artifact is a QA-evidence file (Story 4.12 persistence path) or a retry-round artifact (Story 5.5 externalization path). The marker-class identifier is unchanged; the load-bearing reuse posture is preserved.

3. **Sub-classifications `qa-evidence` vs `retry-history`** per Pattern 2's `: <cause>` suffix format (architecture.md line 962). Both sub-classifications are enumerated in `marker-taxonomy.yaml:227` as `[qa-evidence, retry-history]` (alphabetical) — same shape Story 6.5 demonstrated for `cost-near-ceiling`'s `[ceiling-crossed]`. The rendered marker is `dangling-evidence-ref: qa-evidence` (for QA-evidence dangling) OR `dangling-evidence-ref: retry-history` (for retry-history dangling); both appear in alphabetical order when both are present.

4. **Markers render in BOTH the loud-fail block (top-of-bundle, Story 6.1 surface) AND inline at the reference location** (the per-AC body's evidence bullet OR the retry-history per-round bullet). The loud-fail block carries the source-class summary (one marker per source-class regardless of how many individual refs dangle within that class); the per-AC / per-round body carries the per-ref detail via inline `⚠️ dangling-evidence-ref: <sub-classification> — Remediation: regenerate the evidence OR fix the reference (path=...)` indicators.

5. **Story 5.5's CLI hook `retry-history-resolve` is preserved as a complementary surface** (not deprecated). The CLI fires at hook-execution time; the assembler-side validation fires at bundle-render time. Both reduce to the same marker class. The bundle-assembler-side validation catches refs that danglize between hook execution and bundle render, AND catches QA-evidence refs which Story 5.5's CLI does NOT cover.

6. **Bundle assembly does NOT halt on dangling refs — visibility-not-enforcement per the loud-fail doctrine.** NFR-O7 says "or emits a dangling-ref loud-fail marker" — it does NOT say "or the bundle fails to assemble." The bundle assembles successfully on dangling; the practitioner sees the markers in the loud-fail block AND inline at the reference locations and decides remediation. This mirrors Story 6.5's no-auto-halt on cost ceiling crossing AND Story 6.4's graceful-degrade on OTel-pipeline failure — the loud-fail-doctrine posture ratified across Epic 6.

7. **Phase-2 enforcement-mode thickening is a forward-looking FR-P2 candidate** revisited when practitioner-UX feedback informs the right policy. Examples: evidence regeneration on detection; bundle-block on dangling refs; auto-restoration from a content-addressable store. None of these are MVP scope; the visibility-mode surface IS the MVP.

**Composition discipline.** Story 6.6 is a NEW SIBLING substrate module (`evidence_linkability.py`) that COMPOSES with existing surfaces (Story 4.12's `EVIDENCE_ROOT`, Story 5.5's `RetryAttemptRef` / `detect_dangling_refs`, Story 4.7/4.8's `$defs/evidence_ref`) without modifying them. The only schema-artifact touched is `marker-taxonomy.yaml` for ONE additive `sub_classifications` enum extension per the convention 6.5 demonstrated. The `_render_per_ac_section` and `_render_retry_history` augmentations preserve byte-stable output for callers without dangling refs (default-empty `qa_evidence_dangling` / `retry_history_dangling` keyword-only parameters). The `_merge_evidence_linkability_markers` helper de-duplicates against `dangling-evidence-ref:`-prefixed entries already in `active_markers` so a prior CLI-hook emission is preserved at its original position per Story 1.4's marker-permanence rule.

**Default repo-root for evidence resolution.** `assemble_bundle`'s `resolved_repo_root` defaults to `pathlib.Path.cwd()` for the evidence-trace linkability validation path (NOT `find_repo_root()`). Rationale: production hooks invoke the assembler from the user's project root (where evidence files live), not from the harness install location. Tests override via the `repo_root` keyword arg. The schema-loading path (`load_schema(find_repo_root() / ...)`) is unchanged — the harness's own schema artifacts ARE always anchored to the harness install.

**Cross-references.** This addendum is a clarification of NFR-O7 + Pattern 2 + the marker-class-reuse principle (Story 1.11) ratified by Story 5.5 — NOT a new architectural decision. No new ADR is introduced. The CLI-side complementary surface lives at `tools/loud-fail-harness/src/loud_fail_harness/retry_history.py` (Story 5.5); the bundle-assembler-side render-time surface lives at `tools/loud-fail-harness/src/loud_fail_harness/evidence_linkability.py` (THIS story); the marker-coverage-audit row at `tools/loud-fail-harness/src/loud_fail_harness/_data/marker_coverage_surfaces.yaml` records the canonical assembler-side emission point at `evidence_linkability.py:534`. Epic 6's remaining stories (6.7 / 6.8 / 6.9) consume the Story-6.6-thickened evidence-linkability surface AS-IS; Epic 8's resumability stories (8.1 / 8.4 / 8.5) consume the same dangling-evidence-ref marker class for orphan-run-state detection per the marker-class-reuse posture extended HERE.

### Orchestrator-side marker wiring — specialist-timeout / hook-failed / context-near-limit persistence + alphabetical render order (Story 6.7 resolution)

Story 6.7 lands the orchestrator-side wiring that turns three previously-raised-but-not-persisted skip events (`specialist-timeout` per NFR-P2, `hook-failed: <hook-name>` per NFR-R6, `context-near-limit: <specialist>` per NFR-P4) into entries in `run_state.active_markers` + `run_state.marker_contexts`, flowing through Story 6.1's loud-fail block with full Story 6.2 actionable-pointer enrichment.

**Canonical recording surfaces.** `tools/loud-fail-harness/src/loud_fail_harness/marker_wiring.py` provides:

- `record_specialist_timeout_marker(*, run_state, specialist, timeout_seconds, sub_cause)` — populates `marker_contexts["specialist-timeout"] = {"specialist", "timeout_seconds"}`.
- `record_hook_failure_marker(*, run_state, hook_name)` — appends `hook-failed: <hook-name>`; `marker_contexts` unchanged (taxonomy `pointer_context_fields: []`).
- `record_context_near_limit_marker(*, run_state, specialist)` — populates `marker_contexts["context-near-limit"] = {"specialist"}`.
- `record_marker_with_context(*, run_state, marker_class, sub_classification?, context?, marker_registry?)` — generic helper composing the three named recorders; ALSO consumed by `qa_evidence_tier.record_tier_3_not_configured_in_run_state` (Story 6.7 D-6.2-1 discharge for `Tier-3-not-configured`'s `{ac_id}` context) AND `playwright_driver.record_playwright_mcp_unavailable_in_run_state` (D-6.2-1 discharge for `playwright-mcp-unavailable`'s `{project_type, version_range}` context).
- `compute_alphabetical_marker_order(active_markers)` — pure render-time normalization helper; sorts by `(base_class, sub_classification)` lexicographically with stable tie-breaking.

**Composition seams (sensor-not-advisor).** `tools/loud-fail-harness/src/loud_fail_harness/specialist_dispatch.py` adds two boundary helpers — `handle_specialist_timeout_at_dispatch_boundary(*, exception, run_state, marker_recorder?)` for the caught `SpecialistTimeoutExceeded` case AND `handle_context_near_limit_at_dispatch_boundary(*, is_near_limit, specialist, run_state, marker_recorder?)` for the dispatch-wrapper's `is_context_near_limit` callable signal source. `tools/loud-fail-harness/src/loud_fail_harness/orchestrator_run_entry.py` adds `handle_hook_exit_code(*, exit_code, hook_name, run_state)` for the hook-result handler. None of the three boundary helpers calls `advance_run_state` — Pattern 4's batch-write rule keeps run-state writes batched at the orchestrator-skill layer.

**Order-stable rendering at the loud-fail block.** Story 6.1's `_render_loud_fail_block` was extended at one iteration seam to wrap `active_markers` in `compute_alphabetical_marker_order(...)`. The persistent on-disk tuple stays in emission order per Story 1.4's marker-permanence rule; only the rendered iteration order is normalized. This split serves two consumers: emission order has provenance value (which marker fired first is sometimes diagnostic), rendered order has UX value (alphabetical scanning is faster than chronological hunting).

**Taxonomy extensions (additive only).** `schemas/marker-taxonomy.yaml` gains TWO entry-level extensions: (1) `hook-failed.sub_classifications` extends from `[non-zero-exit, timeout, missing-binary]` to `[missing-binary, non-zero-exit, session-start, stop, subagent-stop, timeout]` (alphabetical merge — three additive entries; the existing three preserved per marker-permanence). (2) `context-near-limit` gains `pointer_context_fields: [specialist]` + `sub_classifications: [dev, qa, review-bmad]` + an interpolated `diagnostic_pointer` text incorporating `{specialist}` and a remediation hint pointing at this section's "Context-budget management" framing. ZERO new marker classes; the marker-class-reuse principle (Story 1.11) is structurally preserved.

**Sensor-not-advisor about the signal source.** The substrate is intentionally agnostic about WHERE the context-near-limit signal originates. The dispatch-wrapper layer's `is_context_near_limit: Callable[[SpecialistEnvelope], bool] | None = None` parameter accepts (a) an explicit envelope field, (b) a runtime token-count estimate, (c) a Claude Code primitive callback, or (d) some other future signal — and supplies the boolean. Phase 2 thickening (e.g., a canonical envelope field; an automated probe) is a forward-looking FR-P2 candidate revisited when the practitioner-UX feedback informs the right policy.

**Visibility-not-enforcement per the loud-fail doctrine.** The three target marker classes surface as loud-fail-block entries; the bundle assembles successfully. NFR-P2 / NFR-P4 / NFR-R6 do not say "or the loop halts" — they say the marker is observable. The practitioner reads the loud-fail block and decides remediation. This mirrors Story 6.5's no-auto-halt on cost ceiling crossing, Story 6.6's no-halt on dangling refs, and Story 6.4's graceful-degrade on OTel-pipeline failure — the Epic-6-ratified posture.

**Context-budget management (NFR-P4 remediation reference).** When a `context-near-limit: <specialist>` marker fires, the practitioner's remediation options are: (1) split the AC into smaller increments so the specialist's working context stays within budget; (2) reduce input scope (trim irrelevant story-doc sections from the dispatch payload); (3) accept the degradation and let the orchestrator's retry-mode select `fix-only` rather than `from-scratch` (the `context-near-limit` marker IS the signal that drives this retry-mode choice — see Story 5.3's `retry_mode` field for the contract pair). Phase 2 thickening (e.g., automatic AC-splitting; specialist-prompt slimming) is a forward-looking FR-P2 candidate.

**Composition discipline.** Story 6.7 is a NEW SIBLING substrate module (`marker_wiring.py`) that COMPOSES with existing surfaces (Story 2.6's `SpecialistTimeoutExceeded`, Story 2.7's hook scaffolds, Story 6.1's `_render_loud_fail_block`, Story 6.2's `_interpolate_actionable_pointer`) without modifying their core contracts. The four thickening flags (`thickening_flags.py`) remain `True` post-6.7. ZERO new marker classes; ZERO new event classes; ZERO schema modifications beyond the two additive `marker-taxonomy.yaml` entry extensions. The in-place-flip / additive-substrate discipline ratified by Stories 3.4 / 4.13 / 5.9 / 6.1 / 6.2 / 6.3 / 6.4 / 6.5 / 6.6 is preserved.

**Cross-references.** This addendum is a clarification of NFR-P2 + NFR-P4 + NFR-R6 + Pattern 2 + Pattern 4 + Pattern 5 + the marker-class-reuse principle (Story 1.11) — NOT a new architectural decision. The dispatch-boundary helpers live at `tools/loud-fail-harness/src/loud_fail_harness/specialist_dispatch.py` (specialist-timeout + context-near-limit); the hook-result handler lives at `tools/loud-fail-harness/src/loud_fail_harness/orchestrator_run_entry.py` (hook-failed); the canonical recorders + alphabetical normalizer live at `tools/loud-fail-harness/src/loud_fail_harness/marker_wiring.py`; the D-6.2-1 discharge helpers live at `tools/loud-fail-harness/src/loud_fail_harness/qa_evidence_tier.py` (Tier-3-not-configured) AND `tools/loud-fail-harness/src/loud_fail_harness/playwright_driver.py` (playwright-mcp-unavailable). The marker-coverage-audit data file at `tools/loud-fail-harness/src/loud_fail_harness/_data/marker_coverage_surfaces.yaml` flips seven rows from `scheduled-by-story` to `emitted` with canonical code-path references. Story 5.5's complementary CLI surface for `dangling-evidence-ref` (separate marker class) is unchanged. Epic 6's remaining stories (6.8 / 6.9) consume the Story-6.7-thickened wiring AS-IS; Epic 7's installation flows + Epic 8's resumability stories consume the same recording-helper convention via the canonical `record_marker_with_context` for any future orchestrator-side marker classes.
...
**Composition discipline.** Story 6.8 is a NEW SIBLING substrate module (`fr33_runtime_gate.py`) that COMPOSES with existing surfaces (Story 1.4's `reconciler.reconcile` + `load_marker_taxonomy`; Story 1.3's `validate_event` + `format_errors`; Story 2.12's `default_event_log_path`; Story 2.2's run-state schema) without modifying their core contracts. The four thickening flags (`thickening_flags.py`) remain `True` post-6.8. ZERO new marker classes (`reconciler-mismatch-runtime` was reserved in the v1 closed taxonomy by Story 1.4); ZERO new event classes; ZERO schema modifications beyond the ONE additive `marker-taxonomy.yaml` entry extension. The in-place-flip / additive-substrate discipline ratified by Stories 3.4 / 4.13 / 5.9 / 6.1 / 6.2 / 6.3 / 6.4 / 6.5 / 6.6 / 6.7 is preserved.

**Cross-references.** The runtime gate module is at `tools/loud-fail-harness/src/loud_fail_harness/fr33_runtime_gate.py`; the sibling fixture gate is at `tools/loud-fail-harness/src/loud_fail_harness/fr33_fixture_gate.py` (UNCHANGED). The shared reconciler is at `tools/loud-fail-harness/src/loud_fail_harness/reconciler.py:147-191` (UNCHANGED). The capture-fixture corpus lives at `tools/loud-fail-harness/tests/fixtures/runtime-captures/` with the synthesis convention documented at the corpus README. The new pyproject entry-point `fr33-runtime-gate` AT `tools/loud-fail-harness/pyproject.toml` mirrors `fr33-fixture-gate`. The new CI step at `bmad-autopilot/.github/workflows/ci.yml` runs immediately after the existing FR33 fixture-driven gate step. The marker-coverage-audit data file at `tools/loud-fail-harness/src/loud_fail_harness/_data/marker_coverage_surfaces.yaml` flips ONE row (`reconciler-mismatch-runtime × reconciliation-gate-runtime`) from `scheduled-by-story: "6.8"` to `verdict: emitted`; emitted-count post-6.8 is 30 (was 29 post-6.7); scheduled-by-story-count post-6.8 is 7 (was 8 post-6.7; remaining: 6.9 + Epics 7-8).

### Bundle-assembly failure detection — three-channel atomic emission for `bundle-assembly-failed` (Story 6.9 resolution)

Story 6.9 lands the orchestrator-side substrate library that detects bundle-assembly logical failures at `bundle_assembly.main`'s entry-point seam and surfaces them via the `bundle-assembly-failed` marker across three reinforcing channels. The new `tools/loud-fail-harness/src/loud_fail_harness/bundle_assembly_failure.py` is a substrate library (sibling of `review_layer_failure.py` / `marker_wiring.py` / `cost_telemetry.py` / `cost_streaming.py` / `evidence_linkability.py`); it is NOT a sixth substrate component beyond ADR-003's enumerated five at architecture.md lines 311-315.

1. **Three reinforcing channels (FR59 + NFR-O5).** A single source-of-truth function `surface_assembly_failure()` produces all three channels atomically: (1) the **fallback diagnostic file** at `_bmad-output/pr-bundles/{story_id}/{run_id}.assembly-failure.log` (always; structured-text content with header `=== bundle-assembly-failed ===`, `story_id`, `run_id`, `failed_step`, `exception_type`, `exception_message`, full Python `traceback`, optional `partial_bundle_path`, ISO-8601 UTC `generated_at`); (2) the **streaming terminal** one-line stderr emission `bundle-assembly-failed: <failed-step> at <story-id>/<run-id> — see <fallback-diagnostic-path>` (always); (3) the **persisted run-state marker** via `record_marker_with_context` + `advance_run_state` so the next-cycle bundle (after remediation) renders the marker in its loud-fail block via the existing Story 6.1 + Story 6.2 path. The "best-effort partial-render" sub-projection of Channel 3 is naturally satisfied by the partial bundle's loud-fail block IF the assembler reached the render step before crashing — no special-casing required.

2. **Source-of-truth-with-CI-lint structural enforcement (Story 3.3 precedent).** The atomicity of the three channels is enforced as a CODE-STRUCTURE invariant via the new CI lint at `tools/loud-fail-harness/src/loud_fail_harness/bundle_assembly_failure_emission_gate.py` mirroring Story 3.3's `review_layer_failure_emission_gate.py` byte-for-byte in shape. The gate scans the harness source tree for forbidden direct emissions: any string literal `"bundle-assembly-failed"` outside the source-of-truth module is a violation; any direct mention of the `*.assembly-failure.log` suffix outside the source-of-truth module is a violation. No developer can emit one channel without the others — same pattern Story 3.3 ratified for `surface_failed_layers` and Story 2.2 ratified for atomic-write (the API shape IS the invariant, not a per-bundle reconciliation gate).

3. **Exit-code-2 convention distinguishing assembler-logic vs. Stop-hook-mechanical failure (AC-3).** The `bundle-assembly-failed` and `hook-failed: stop` markers stay remediation-shape-distinct at runtime via a new `BUNDLE_ASSEMBLY_FAILED_EXIT_CODE: Final[int] = 2` constant exported from `bundle_assembly_failure.py`. The augmented `bundle_assembly.main`'s outer `try/except` returns exit 2 after invoking `surface_assembly_failure` (the marker is already emitted via Channel 3); the existing `handle_hook_exit_code` helper (Story 6.7) gains a new conditional that does NOT invoke `record_hook_failure_marker` when `hook_name == "stop"` AND `exit_code == BUNDLE_ASSEMBLY_FAILED_EXIT_CODE`. Cross-failure independence is preserved: when both surfaces fail (Stop hook crashes mechanically AND assembler also crashed), both markers fire — `bundle-assembly-failed` from the assembler's own failure path and `hook-failed: stop` from the helper because the hook's exit code is non-zero AND not equal to 2 (e.g., 1 from `set -e` failure before/after `exec`, or 127 from missing `python3`).

4. **Pre-condition failures stay at exit 1 without emitting `bundle-assembly-failed`.** `SpecialistDispatchLogNotFound` and `RunStateStoryIdMismatch` are pre-condition failures — the assembler had nothing to assemble (the dispatch log or run-state didn't exist or didn't match) — and remain in `bundle_assembly.main`'s inner triage at exit 1 WITHOUT emitting the marker. The `bundle-assembly-failed` marker covers assembler-logic failures (envelope shape mismatch, missing finding fields, taxonomy reference unresolved, finding-rendering crash, assembler-internal exception per epics.md line 2818 verbatim), NOT pre-condition contract violations. The five enumerated assembler-failure modes are carried as the marker's `sub_classifications` per the additive `marker-taxonomy.yaml` extension; `pointer_context_fields: [exception_type, failed_step, run_id, story_id]` populated by `surface_assembly_failure`'s `marker_contexts` write feeds the Story 6.2 actionable-pointer interpolation on the next-cycle bundle.

5. **Phase 2 thickening (forward-looking).** Two thickening directions are tracked in `deferred-work.md`: (a) **Compound sub-classifications** combining `failed_step` AND specific finding-field name (e.g., `bundle-assembly-failed: missing-finding-fields/severity`) for finer-grained remediation pointing — currently the `failed_step` carries the granularity and the specific field surfaces in the fallback diagnostic file's `traceback` block. (b) **Graceful-degradation rendering** — instead of returning exit 2 after the first uncaught assembler exception, render a maximally-partial bundle plus the marker so practitioners see whatever DID assemble; the loud-fail block already supports the marker via Channel 3's persisted form, but the partial-render path requires careful handling of mid-render mutations that the current single-shot `assemble_bundle` does not separate. These are forward-looking FR-P2 candidates revisited when practitioner-UX feedback informs the right policy.

**Composition discipline.** Story 6.9 is a NEW SIBLING substrate library (`bundle_assembly_failure.py`) that COMPOSES with existing surfaces (Story 2.11's `bundle_assembly.main`, Story 6.7's `handle_hook_exit_code`, Story 6.7's `marker_wiring.record_marker_with_context`, Story 1.4's marker taxonomy) without modifying their core contracts. The four thickening flags (`thickening_flags.py`) remain `True` post-6.9. ZERO new marker classes (`bundle-assembly-failed` was reserved in the v1 closed taxonomy by Story 1.4); ZERO new event classes; ZERO schema modifications beyond the ONE additive `marker-taxonomy.yaml` entry extension (`pointer_context_fields` + `sub_classifications` + interpolated `diagnostic_pointer`). The in-place-flip / additive-substrate discipline ratified by Stories 3.4 / 4.13 / 5.9 / 6.1 / 6.2 / 6.3 / 6.4 / 6.5 / 6.6 / 6.7 / 6.8 is preserved.

**Cross-references.** The substrate library is at `tools/loud-fail-harness/src/loud_fail_harness/bundle_assembly_failure.py`; the CI-lint gate is at `tools/loud-fail-harness/src/loud_fail_harness/bundle_assembly_failure_emission_gate.py`. The augmented assembler entry-point is at `tools/loud-fail-harness/src/loud_fail_harness/bundle_assembly.py:main`; the augmented hook-result handler is at `tools/loud-fail-harness/src/loud_fail_harness/orchestrator_run_entry.py:handle_hook_exit_code`. The taxonomy entry extension lives at `schemas/marker-taxonomy.yaml` `bundle-assembly-failed`. The new pyproject entry-point `bundle-assembly-failure-emission-gate` at `tools/loud-fail-harness/pyproject.toml` mirrors `review-layer-failure-emission-gate`. The new CI step at `bmad-autopilot/.github/workflows/ci.yml` runs immediately after the existing review-layer-failure-emission-gate step. The marker-coverage-audit data file at `tools/loud-fail-harness/src/loud_fail_harness/_data/marker_coverage_surfaces.yaml` flips TWO rows (`bundle-assembly-failed × bundle-assembler` and `bundle-assembly-failed × hook-stop`) from `scheduled-by-story: "6.9"` to `verdict: emitted`. The Stop hook script at `hooks/stop.sh` is UNCHANGED — the hook's exit code is the runtime signal consumed by `handle_hook_exit_code`. The canonical assembly-failure fixture at `examples/assembly-failures/sample-assembly-failure.log` documents the structured-text format. Epic 6 closes with all 9 stories `done`; the in-place-flip discipline carries into Epic 7 (installation + onboarding) and Epic 8 (resumability + multi-story inspection).

## QA Behavioral Plan Per-AC Flow-Branch Enumeration (FR22c / Story 13.2)
...
**`intentionally-skipped` → `heuristic-skipped: flow-branch-<branch-id>` marker path.** The new `surface_flow_branch_skipped(story_id, ac_id, branch, registry)` helper is a Pattern-5 atomic-on-failure emission helper mirroring Story 4.6's `surface_smoke_first_abort` and Story 4.9's `surface_heuristic_skipped` byte-for-byte: it calls `validate_marker_emission(registry, HEURISTIC_SKIPPED_MARKER)` FIRST, so a registry lacking the marker class raises `UnknownMarkerClass` atomically before any model is constructed. It then constructs a `FlowBranchSkippedEmissionRecord` carrying `marker_class="heuristic-skipped"` (the EXISTING v1 27-class marker class — NO new top-level class), `sub_classification="flow-branch"` (the taxonomy sub-classification Story 13.6's v1.5 → v1.6 PATCH bump declares), and a five-field `FlowBranchSkippedDiagnosticContext` (`story_id` / `ac_id` / `branch_id` / `branch_description` / `skip_rationale`, the rationale copied verbatim from the `FlowBranch`). The FR22c-canonical rendered marker token `heuristic-skipped: flow-branch-<branch-id>` is composed from `marker_class` + `sub_classification` + `branch_id` by the bundle assembler — `surface_flow_branch_skipped` is pure (no I/O, no orchestrator-event emission); it RETURNS the structured record, exactly as the two precedent helpers do. The three new emission models (`FlowBranchSkippedDiagnosticContext` / `FlowBranchSkippedEmissionRecord` / `FlowBranchSkippedEmission`) are local to Story 13.3 — NOT a reuse of Story 4.9's `HeuristicSkipped*` trio — per the cross-story-coupling-avoidance posture: a within-AC flow-branch skip and a cross-AC exploratory-heuristic skip share the marker class but carry different diagnostic payloads.

**`must-visit` → `AcFlowBranchCoverage.must_visit_branch_ids` obligation — the library does NOT drive branches.** The other half of the FR22c contract is surfaced as typed data: every `must-visit` branch's `branch_id` lands in `AcFlowBranchCoverage.must_visit_branch_ids`, carried back on the new additive-optional `AcIterationResult.flow_branch_coverage: tuple[AcFlowBranchCoverage, ...]` field (appended LAST, `Field(default_factory=tuple)` — non-breaking for every pre-existing constructor and consumer). `iterate_acs` is a pure library with no LLM and no per-branch driver surface: it does NOT itself visit or drive a `must-visit` branch. Driving each `must-visit` branch and supplying per-branch evidence is the QA agent's run-time responsibility, instructed by Story 13.4's `agents/qa.md` wrapper-prompt change; the obligation-vs-evidence reconciliation — a `must-visit` branch with no evidence and no marker is a contract violation — is Story 13.5's multi-branch CI fixture gate. `iterate_acs`'s contract role is exactly two things: emit the deterministic, plan-derivable skip markers, and surface the `must-visit` obligation as typed data.

**Smoke-first-abort precedence (AC-6 — deferred-work resolution).** This paragraph is the canonical resolution of the deferred-work item "FR22b × FR22c interaction at AC-1 failure unspecified" (`_bmad-output/implementation-artifacts/deferred-work.md`, the 13-1 code-review block). Smoke-first abort (FR22b) takes precedence over flow-branch processing: when AC-1's `verify_ac` returns `status == "fail"`, `iterate_acs` fires `surface_smoke_first_abort` and returns IMMEDIATELY — AC-1's own `flow_branches[]` are NOT processed (no skip markers, no coverage record), no downstream AC is iterated, and the returned `AcIterationResult` carries `flow_branch_coverage == ()`. A broken happy path makes within-AC branch coverage moot, exactly as FR22b makes downstream-AC coverage moot. For every NON-aborting AC — a `pass` or `blocked` AC-1, and every reached AC-2..AC-N — flow-branch processing runs regardless of that AC's own `status`: the `intentionally-skipped` emissions are plan-derived and status-independent, so a `blocked` AC or a mid-iteration `fail` AC still has its branches accounted for in the bundle.

**Operator edit to `flow_branches[]` between runs (deferred-work resolution).** The residual clause of the deferred-work item "ac_hash and flow_branches[] boundary unspecified" — the run-time reuse-vs-regenerate consequence routed to Story 13.3 — resolves as follows: because `flow_branches[]` is OUT of `compute_ac_hash` scope (Story 13.2), an operator editing `flow_branches[]` between runs does NOT change `ac_hash` and so does NOT trigger Story 4.2's `plan-drift-detected`; the plan is reused with the edited `flow_branches[]` and `iterate_acs` consumes whatever the reused (or regenerated) plan carries, verbatim. This is the correct behavior and needs no code in Story 13.3 — an operator refining the branch enumeration is making an intentional plan edit that should be honored, not flagged as AC drift.
...
**The structural counterweight.** Story 13.5 is the Story-1.7/1.8-pattern blocking, fixture-driven, CI-enforced verification surface for the FR22c contract scaffolded by Stories 13.2 (the `flow_branches[]` schema), 13.3 (the `iterate_acs` iteration contract), and 13.4 (the `agents/qa.md` wrapper prompt) — exactly the structural-enforcement counterweight to the sensor-not-advisor invariant named by Architecture Pattern 8. A regression that drops a branch, mis-dispositions one, leaves a `must-visit` branch undischarged, or emits a `flow-branch` sub-classification the taxonomy does not declare fails the build loudly instead of silently shipping under-tested optional flows. The gate REPLAYS an emission helper for reconciliation — it is NOT itself a production marker-emission surface, so it introduces NO new marker class and NO entry in `marker_coverage_surfaces.yaml` (`fr33-fixture-gate`'s posture: it replays `reconcile()` without being a reconciliation surface). No schema is touched anywhere in Story 13.5 — `marker-taxonomy.yaml` (Story 13.6), `qa_ac_iteration.py` (Story 13.3), and `qa_behavioral_plan.py` (Story 13.2) are all consumed AS-IS.

**Cross-references.** The gate module is at `tools/loud-fail-harness/src/loud_fail_harness/flow_branch_coverage_gate.py`; its test suite at `tools/loud-fail-harness/tests/test_flow_branch_coverage_gate.py`. The fixture corpus is at `tools/loud-fail-harness/tests/fixtures/flow-branch-coverage/` (plus the `flow-branch-coverage-failure-cases/` sibling), documented by that directory's `README.md`. The `flow-branch-coverage-gate` `[project.scripts]` entry is in `tools/loud-fail-harness/pyproject.toml`; the CI step is the final step of `bmad-autopilot/.github/workflows/ci.yml`. This addendum is a clarification of FR22c + Pattern 8 + the Sprint Change Proposal 2026-05-20 — NOT a new architectural decision; no new ADR is introduced and the existing addenda above (including Story 13.2's and Story 13.3's) are not modified. Story 13.5 closes the `(CI fixture)` step of the Epic 13 chain; only Story 13.7 (reference-run refresh) remains.
