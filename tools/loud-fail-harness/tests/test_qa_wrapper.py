"""Contract-coverage matrix for the QA specialist wrapper (Story 2.10 lander
+ Story 4.13 wrapper-thickening completion).

This docstring IS the contract-coverage checklist required by Story 2.10's
AC-7. Reviewers verify every row maps to at least one passing test in this
module. The matrix is review-enforced, NOT CI-enforced (parallel to stories
1.2-1.9 + 2.2-2.9).

Story 4.13 RELAXED IN PLACE the wrapper-prose tests that assert Story-2.10-
era AC-1-only / Tier-1-only framing — those framings are no longer the
canonical wrapper surface post-thickening. The Story-2.10 fixture-shape
tests are PRESERVED (the canonical envelope fixtures `qa-pass-ac1-tier1.yaml`
+ `qa-fail-ac1-assertion.yaml` retain their Story-2.10 ac_results-length-1
shape — they are not regenerated by Story 4.13 per its scope-discipline
clause). The wrapper-prose tests are relaxed to assert the post-4.13
canonical surface (full FR16-FR25; smoke-first-abort framing for AC-1
anchoring; per-AC ac_results cardinality matching len(ac_list)).

Fixture-shape conformance (Story 2.10 AC-7 — fixtures unchanged at Story 4.13):
...
Cross-fixture invariants (Story 2.10 AC-4, AC-7, FR52, FR55 — fixtures unchanged):
...
Wrapper-prose discipline (Story 2.10 AC-2, AC-3, AC-4, AC-5, AC-6, AC-8 +
Story 4.13 AC-1 wrapper-thickening completion):
    [x] qa.md anchors AC-1 in smoke-first-abort framing (post-4.13)→ test_qa_wrapper_documents_ac1_smoke_first_anchor
    [x] qa.md documents post-4.13 ac_results cardinality + tiers  → test_qa_wrapper_documents_ac_results_invariant
...
def test_qa_wrapper_documents_ac1_smoke_first_anchor(qa_wrapper_text: str) -> None:
    """Story 4.13 AC-1 relaxation of Story 2.10's
    ``test_qa_wrapper_documents_ac1_only_single_scope``. Post-thickening,
    the AC-1 anchoring shifts from Epic-2-AC-1-only-scope framing to the
    smoke-first-abort framing of FR22b (`iterate_acs` enforces AC-1
    first; AC-1 failure aborts the iteration). The new assertion:
    within 600 chars of the first AC-1 mention, the wrapper names
    either "smoke-first" (the FR22b anchor) or "FR22b" (the verbatim FR
    identifier).
    """
    ...
    assert re.search(r"smoke-first|FR22b", window_600), (
        "first AC-1 mention must name smoke-first / FR22b within 600 chars "
        "(post-4.13 anchoring per Story 4.6's iterate_acs framing)"
    )


def test_qa_wrapper_documents_ac_results_invariant(qa_wrapper_text: str) -> None:
    """Story 4.13 AC-1 relaxation of the Story-2.10-era cardinality-1
    assertion. Post-thickening, ac_results cardinality grows from
    "exactly one entry" to "one entry per iterated AC in story-doc
    order" (smoke-first abort produces a single AC-1 entry whose
    status: fail; the no-abort path emits len(ac_list) entries). The
    new assertions: the wrapper mentions ac_results AND documents the
    post-4.13 cardinality growth somewhere in its prose AND documents
    the AC-assertion-evidence triple per Story 4.7.
    """
    text = qa_wrapper_text
    assert "ac_results" in text, "qa.md must mention ac_results"
    assert re.search(
        r"one entry per iterated AC|len\(ac_list\)|ac_list length", text
    ), (
        "qa.md must document the post-4.13 cardinality growth (one entry per "
        "iterated AC / ac_list length / len(ac_list))"
    )
    assert "triple" in text, (
        "qa.md must document the AC-assertion-evidence triple per Story 4.7"
    )
