...
CLI / main exit-code matrix (AC-4, AC-6):
    [x] canonical corpus validates → exit 0 + 37 Summary line  → test_canonical_corpus_validates
...
def test_pr_introduces_reconciler_regression(
    ...
    """Epic AC #2: PR regresses reconciler → ALL 37 fixtures hit reconciliation_mismatch."""
    ...
    rc, out, _ = _capture_main([])
    assert rc == 1
    assert "40 reconciliation-mismatch finding(s)" in out
...
def test_canonical_corpus_validates() -> None:
    """The canonical 37-fixture corpus + canonical schemas → exit 0 (story
    ...
    story 19.3 added 3 markers + 3 fixtures (34 → 37))."""
    rc, out, err = _capture_main([])
    assert rc == 0, f"stdout: {out}\nstderr: {err}"
    assert "Summary: 40 passing fixture(s)" in out
