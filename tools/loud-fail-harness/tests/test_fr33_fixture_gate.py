...
CLI / main exit-code matrix (AC-4, AC-6):
    [x] canonical corpus validates → exit 0 + 32 Summary line  → test_canonical_corpus_validates
...
def test_pr_introduces_reconciler_regression(
    ...
    """Epic AC #2: PR regresses reconciler → ALL 32 fixtures hit reconciliation_mismatch."""
    ...
    rc, out, _ = _capture_main([])
    assert rc == 1
    assert "32 reconciliation-mismatch finding(s)" in out
...
def test_canonical_corpus_validates() -> None:
    """The canonical 32-fixture corpus + canonical schemas → exit 0 (story
    2.3 added 2 markers + 2 fixtures (27 → 29); story 14.3 added 1 marker
    + 1 fixture (29 → 30); story 14.5 added 1 marker + 1 fixture (30 → 31);
    story 15.2 added 1 marker + 1 fixture (31 → 32))."""
    rc, out, err = _capture_main([])
    assert rc == 0, f"stdout: {out}\nstderr: {err}"
    assert "Summary: 32 passing fixture(s)" in out
